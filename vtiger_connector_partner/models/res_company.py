...
class ResCompany(models.Model):
    ...
    def fetch_data(self, company, vtiger_type):
        ...
        if company.last_sync_date:
            qry = qry_template[vtiger_type].format((company.last_sync_date) - timedelta(minutes=1))
        else:
            ...
            for res in result.get('result', []):
                ...
                if vtiger_type in ('Contacts', 'Vendors', 'Accounts'):
                    if partner_vals.get('country_id'):
                        ...
                        # country = country_obj.filtered(lambda c: c.name == partner_vals.get('mailingcountry') or lambda c: c.code == partner_vals.get('mailingcountry'))
                        if country:

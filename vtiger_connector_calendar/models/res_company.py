...
##                    setting the time duration
                    get_hours = res.get('duration_hours')
                    get_minutes = res.get('duration_minutes')
...
#                        })
                        ...
                        date_s = datetime.strptime(get_date, DEFAULT_SERVER_DATE_FORMAT) + timedelta(hours=hour,minutes=minute,seconds=second)
                        date_stp = date_s + timedelta(days=1)
                        duration = calendar_obj._get_duration(str(date_s), str(date_stp))
                        calendar_vals.update({
                            'start': str(date_s),
